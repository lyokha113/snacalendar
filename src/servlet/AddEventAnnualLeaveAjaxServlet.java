 ...
 * @author Yuu
 */
@WebServlet(name = "AddEventAnnualLeaveAjaxServlet", urlPatterns = {"/AddEventAnnualLeaveAjaxServlet"})
public class AddEventAnnualLeaveAjaxServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (eventDate != null && !eventDate.isEmpty() && eventContent != null && !eventContent.isEmpty()) {
                ...
                eventType.setEventTypeID(EventTypeEnum.ANNUAL_LEAVE.getEventTypeID());
                ...
                EventDTO event = new EventDTO(creator, eventType, eventTitle, eventContent,
                        null, null, eventStartDate, eventEndDate);

                // Set status approving for staff and approved for boss
                int status = emp.getTitle().getPermission().getPermissionID() == PermissionEnum.SUPER_USER.getPermissionID()
                        ? AnnualStatusEnum.APPROVED.getStatus()
                        : AnnualStatusEnum.APPROVING.getStatus();

                result = addEventAnnual(event, status) > 0 ?
    ...
    private int addEventAnnual(EventDTO dto, int status) throws SQLException, NamingException {
        EventAnnualDAO dao = new EventAnnualDAO();
        return dao.addEventAnnual(dto, status);
    }
