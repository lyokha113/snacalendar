 ...
 * @author Yuu
 */
@WebServlet(name = "AddEventCustomerCareAjaxServlet", urlPatterns = {"/AddEventCustomerCareAjaxServlet"})
public class AddEventCustomerCareAjaxServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (eventTitle != null && !eventTitle.isEmpty() && eventDate != null && !eventDate.isEmpty()
                ...
                eventType.setEventTypeID(EventTypeEnum.CUSTOMER_CARE.getEventTypeID());
                ...
                // Add event with car or without car
                if (useCar != null && eventCar != null && !eventCar.isEmpty()) {
                    event.setEventID(addEventCar(event, Integer.parseInt(eventCar)));
                } else {
    ...
    private int addEventCar(EventDTO event, int carID) throws SQLException, NamingException {
        EventCarDAO dao = new EventCarDAO();
        return dao.addEventCar(event, carID);
    }
}
